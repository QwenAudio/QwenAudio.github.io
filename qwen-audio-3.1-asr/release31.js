(() => {
  ...
  const bench=[['Moss-Transcribe-Diarize',4.20,16.04,3.38,17.23,3.20,14.06],['Cascade',8.45,21.98,9.03,34.68,6.17,33.83],['Qwen-Audio-3.1-ASR',2.78,14.12,2.02,11.77,1.84,13.66],['Gemini-3.5-Transcribe',13.99,37,21.48,45.16,12.71,23.05],['muse-voice-transcribe-1.0',3.04,19.82,6.16,27.97,3.48,14.60]];
  $('#diar-benchmark').innerHTML=`<table><thead><tr><th rowspan="2" scope="col">${t('Model / approach','模型 / 方案')}</th>${['AISHELL-4','AliMeeting','MLC-SLM'].map(s=>`<th colspan="2" scope="colgroup">${s}</th>`).join('')}</tr><tr>${Array(3).fill('<th scope="col">DER ↓</th><th scope="col">cpWER ↓</th>').join('')}</tr></thead><tbody>${bench.map((r,i)=>`<tr class="${i===2?'ours31':''}"><th scope="row">${r[0]==='Cascade'?t('Cascade','级联方案'):r[0]}</th>${r.slice(1).map(x=>`<td>${x.toFixed(2)}</td>`).join('')}</tr>`).join('')}</tbody></table>`;
