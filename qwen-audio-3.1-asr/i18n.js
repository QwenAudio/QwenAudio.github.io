(() => {
  const zh = {
    ...
    'Fast & streaming':'快速流式识别','Listen while the transcript appears.':'边听音频，边看文字实时出现。','Play the 18-second Beijing-to-Hangzhou request and watch the transcript appear continuously in a clean side-by-side streaming interface.':'播放 18 秒的北京至杭州出行请求，在清晰的左右对比界面中观察文字连续生成。','Interactive streaming playback':'交互式流式播放','Doubao Voice Input':'豆包语音输入法','vs':'对比','Beijing → Hangzhou travel request':'北京 → 杭州出行请求','Voice-timbre adjusted · original comparison replay':'音色已调整 · 原始对比回放','DOUBAO VOICE INPUT':'豆包语音输入法',
    ...
    'Dialect CER':'方言 CER','Consistency':'一致性','Industry recall':'行业实体召回','Polishing':'转写润色','Latency':'延迟','Internal dialect suite':'内部方言评测集','Macro CER across 16 dialects':'16 种方言宏平均 CER','Qwen-Audio-3.1-ASR reports the lowest CER on 11 of 16 test sets, compared with macro CERs of 16.77% for Doubao-ASR and 20.40% for Tencent Hy-ASR-3.0-preview.':'Qwen-Audio-3.1-ASR 在 16 个测试集中的 11 个取得最低 CER；其宏平均 CER 为 9.40%，相比之下 Doubao-ASR 为 16.77%，Tencent Hy-ASR-3.0-preview 为 20.40%。','Lower is better':'越低越好','16 varieties':'16 种方言','Internal benchmark':'内部评测','Task consistency':'任务一致性','Consistency decision threshold':'一致性判定阈值','A judge model scores semantic and task consistency from 0 to 10. Outputs scoring at least 6 are counted as consistent.':'评判模型从 0 到 10 对语义与任务一致性打分，得分不低于 6 的输出被计为一致。','Higher is better':'越高越好','Judge-based':'模型评判','Industry entity recognition':'行业实体识别','Domains led in entity recall':'实体召回率领先的领域','Outright-leading domains':'实体召回率单独领先的领域','Across the internal industry-term evaluation, Qwen-Audio-3.1-ASR reports the highest entity recall in every evaluated domain.':'在内部行业术语评测中，Qwen-Audio-3.1-ASR 在全部评测领域均取得最高实体召回率。','Across the internal industry-term evaluation, Qwen-Audio-3.1-ASR reports the outright highest entity recall in 11 of 15 domains and ties for the highest recall in one additional domain.':'在内部行业术语评测中，Qwen-Audio-3.1-ASR 在 15 个领域中的 11 个取得单独最高实体召回率，另在 1 个领域并列最高。','Entity recall':'实体召回率','Native polishing':'原生润色','Readability score':'可读性得分','Native single-pass polishing substantially improves readability while maintaining faithfulness, approaching an external post-rewrite baseline.':'原生单次润色在保持忠实度的同时显著提升可读性，效果接近外部后处理改写基线。','Native single-pass polishing improves readability from 2.53 to 3.44 while maintaining a faithfulness score of 3.44, approaching an external post-rewrite baseline.':'原生单次润色将可读性从 2.53 提升至 3.44，同时保持 3.44 的忠实度得分，效果接近外部后处理改写基线。','Single pass':'单次生成','Readable output':'更易阅读','Faithfulness':'忠实度','Streaming recognition':'流式识别','Theoretical first-token latency':'理论首字延迟','The streaming model exposes configurable operating points for the latency–accuracy trade-off on internal Chinese and English industrial test sets.':'流式模型可在内部中英文行业测试集上配置不同工作点，以平衡延迟与准确率。','Lower latency':'更低延迟','Streaming':'流式',
    'Samples from the report and launch material':'技术报告与发布材料示例','See what the controls change.':'直观看到不同控制条件带来的变化。','Explore context corrections, entity recall, hotword gains, and native polishing without leaving the page.':'直接体验上下文纠错、实体召回、热词增益与原生转写润色。','Example type':'示例类型','Long context':'长上下文','Hotwords':'热词','Industry entities':'行业实体','Earlier cue':'前文线索','Hierarchical customization':'分级热词定制','P0 for high-confidence terms. P1 for broader candidates.':'P0 用于高置信度词，P1 用于更广泛的候选词。','Across the supplied launch examples, Qwen-Audio-3.1-ASR reaches 99%+ recall on six P0 categories after hotword conditioning.':'在所提供的发布示例中，加入热词条件后，Qwen-Audio-3.1-ASR 在六个 P0 类别上的召回率超过 99%。','Launch examples':'发布示例','Long-tail entities across practical domains.':'覆盖真实行业场景中的长尾实体。','Shown values are entity recall (%) from the supplied promotional material.':'展示数值为所提供宣传材料中的实体召回率（%）。','Native single-pass polishing':'原生单次转写润色','Cleaner transcripts,':'更干净的转写，','without a rewrite stage.':'无需额外改写阶段。','Fillers, repetitions, explicit self-corrections, and formatting noise are handled inside the recognition pass.':'语气词、重复、明确的自我纠正和格式噪声均在识别过程中直接处理。','Polishing off':'关闭润色','Polishing on · Native':'开启原生润色','Readability':'可读性','Qwen3.6-Plus cascade':'Qwen3.6-Plus 级联方案','Filler removal':'去除语气词','Repetition reduction':'减少重复','Self-correction':'自我纠正','Formatting':'格式规范化',
  ...
  function translateText(node) {
    ...
    if (node.parentElement?.closest('.sample-transcript,.sample-translation,.copy31,.native31,code')) return;
